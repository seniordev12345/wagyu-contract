...
interface IWagyuSwapLottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in WAG per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in WAG token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInWagyu: price of a ticket in WAG
...
/** @title WagyuSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract WagyuSwapLottery is ReentrancyGuard, IWagyuSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInWagyu = 50 ether;
    uint256 public minPriceTicketInWagyu = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInWagyu;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] wagPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInWagyu;
        uint32 finalNumber;
    }
     ...
     * @param _wagTokenAddress: address of the WAG token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _wagTokenAddress, address _randomGeneratorAddress) {
        wagToken = IERC20(_wagTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of WAG to this contract
        uint256 amountWagyuToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInWagyu,
            _ticketNumbers.length
        );

        // Transfer wag tokens to this contract
        wagToken.safeTransferFrom(address(msg.sender), address(this), amountWagyuToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInWagyu += amountWagyuToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInWagyuToTransfer
        uint256 rewardInWagyuToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInWagyuToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        wagToken.safeTransfer(msg.sender, rewardInWagyuToTransfer);

        emit TicketsClaim(msg.sender, rewardInWagyuToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInWagyu) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in WAG for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].wagPerBracket[j] =
                ...
                // A. No WAG to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].wagPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInWagyu - amountToShareToWinners);

        // Transfer WAG to treasury address
        wagToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        wagToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInWagyu += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInWagyu >= minPriceTicketInWagyu) && (_priceTicketInWagyu <= maxPriceTicketInWagyu),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInWagyu: _priceTicketInWagyu,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            wagPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInWagyu: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(wagToken), "Cannot be WAG token");
    ...
    /**
     * @notice Set WAG price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInWagyu: minimum price of a ticket in WAG
     * @param _maxPriceTicketInWagyu: maximum price of a ticket in WAG
     */
    function setMinAndMaxTicketPriceInWagyu(uint256 _minPriceTicketInWagyu, uint256 _maxPriceTicketInWagyu)
        external
        onlyOwner
    {
        require(_minPriceTicketInWagyu <= _maxPriceTicketInWagyu, "minPrice must be < maxPrice");

        minPriceTicketInWagyu = _minPriceTicketInWagyu;
        maxPriceTicketInWagyu = _maxPriceTicketInWagyu;
    }
     ...
     * @param _priceTicket price of a ticket (in WAG)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].wagPerBracket[_bracket];
        } else {
            return 0;
        }
    }
