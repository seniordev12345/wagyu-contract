...
contract WagyuVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 wagAtLastUserAction; // keeps track of wagyu deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Wagyu token
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.wagAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Reinvests WAG tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
     ...
     * @return Expected reward to collect in WAG
     */
     ...
     * @return Returns total pending wag rewards
     */
...
contract VaultOwner is Ownable {
    ...
    WagyuVault public immutable wagVault;

    /**
     * @notice Constructor
     * @param _wagVaultAddress: WagVault contract address
     */
    constructor(address _wagVaultAddress) public {
        wagVault = WagyuVault(_wagVaultAddress);
    }
    ...
    function setAdmin() external onlyOwner {
        wagVault.setAdmin(address(this));
    }
     ...
     */
    function setTreasury(address _treasury) external onlyOwner {
        wagVault.setTreasury(_treasury);
    }
    ...
    function setPerformanceFee(uint256 _performanceFee) external onlyOwner {
        wagVault.setPerformanceFee(_performanceFee);
    }
    ...
    function setWithdrawFee(uint256 _withdrawFee) external onlyOwner {
        wagVault.setWithdrawFee(_withdrawFee);
    }
    ...
    function setWithdrawFeePeriod(uint256 _withdrawFeePeriod) external onlyOwner {
        wagVault.setWithdrawFeePeriod(_withdrawFeePeriod);
    }
    ...
    function inCaseTokensGetStuck(address _token) external onlyOwner {
        wagVault.inCaseTokensGetStuck(_token);
        uint256 amount = IERC20(_token).balanceOf(address(this));
        IERC20(_token).safeTransfer(msg.sender, amount);
    }
