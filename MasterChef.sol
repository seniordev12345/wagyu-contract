...
contract WagyuToken is BEP20('WagyuSwap Token', 'WAG') {
    ...
    external
    {
        ...
        require(now <= expiry, "WAG::delegateBySig: signature expired");
...
contract SyrupBar is BEP20('SyrupBar Token', 'SYRUP') {
    ...
    function safeWagyuTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 wagBal = wag.balanceOf(address(this));
        if (_amount > wagBal) {
            wag.transfer(_to, wagBal);
        } else {
...
interface IMigratorChef {
    // Perform LP token migration from legacy WagyuSwap to WagyuSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to Wagyuswap LP tokens.
    // WagyuSwap must mint EXACTLY the same amount of WagyuSwap LP tokens or
    // else something bad will happen. Traditional WagyuSwap does not
    // do that so be careful!
    function migrate(IBEP20 token) external returns (IBEP20);
}

// MasterChef is the master of Wagyu. He can make Wagyu and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Wagyu is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accWagyuPerShare; // Accumulated WAGs per share, times 1e12. See below.
    }
    ...
    function pendingWagyu(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accWagyuPerShare = accWagyuPerShare.add(wagReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accWagyuPerShare = pool.accWagyuPerShare.add(wagReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                safeWagyuTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function safeWagyuTransfer(address _to, uint256 _amount) internal {
        syrup.safeWagyuTransfer(_to, _amount);
    }
